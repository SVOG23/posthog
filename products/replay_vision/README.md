...
**Scanner template:** a team-wide snapshot of a scanner's prompt, type, filters, sampling, and model configuration. Saving the scanner again updates its template. Deleting the source scanner leaves the template available, while deleting the template does not change scanners previously created from it.
