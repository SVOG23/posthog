...
async def test_failure_streak_pauses_scout_once_and_a_success_resumes_it(ateam, aerrors_skill):
    ...
    assert "timed out after 900s" in config.auto_pause_reason
    paused_at = config.auto_paused_at
    trip = _paused_events(capture)
    assert len(trip) == 1
    assert trip[0].kwargs["properties"]["consecutive_failure_count"] == FAILURE_STREAK_PAUSE_THRESHOLD

    # A failed probe restarts the cooldown but must not re-alert — otherwise the event stops
    # being a count of wedges and becomes a count of doomed runs again.
    await _run_once(failing=True, capture=capture)
    config = await _reload()
    assert config.auto_paused_at is not None and config.auto_paused_at > paused_at
    assert len(_paused_events(capture)) == 1

    # A probe that gets through clears the breaker, so a lane whose cause was fixed recovers
    # without anyone having to un-pause it by hand.
    ...
    assert config.auto_paused_at is None
    assert config.auto_pause_reason == ""
