...
| `random_cohort_branch`   | `{ "cohorts": [ { "percentage": 50, "name?": "A" } ], "sticky_assignment?": false }`. `sticky_assignment: true` buckets on a hash of the person id, so someone who re-enters the workflow lands in the same cohort as before; the default assigns a fresh random cohort on every entry. Percentages should sum to 100 — a shortfall leaves an unrouted remainder, an excess makes later cohorts unreachable.                                                                                                                                                                                                    |
