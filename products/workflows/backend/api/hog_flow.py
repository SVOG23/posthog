...
class HogFlowActionSerializer(serializers.Serializer):
    ...
    config = HogFlowActionConfigField(
        help_text=(
            ...
            "Index N matches the 'branch' edge with index:N; percentages should sum to 100 (an unallocated "
            "remainder routes to the last cohort). sticky_assignment defaults to false, which assigns a fresh "
            "random cohort on every entry; set it true to bucket on a hash of the person id so a person who "
            "re-enters the workflow lands in the same cohort as before. "
