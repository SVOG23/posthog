...
function Scorecards(): JSX.Element {
    const { concentratedRoutes, spreadRoutes, topErrorRoute } = useValues(mcpClusteringLogic)
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-surface-primary border rounded p-3 min-h-[88px] flex flex-col">
                ...
                <span className="text-xs text-muted mt-1">
                    Intent groups with {KPI_MIN_SESSIONS}+ sessions where no single tool covers half the calls. Possible
                    routing drift.
                </span>
            </div>
            <div className="bg-surface-primary border rounded p-3 min-h-[88px] flex flex-col">
...
function ClusterList(): JSX.Element {
    const { sortedClusters, longTail, selectedClusterId } = useValues(mcpClusteringLogic)
    const { selectCluster } = useActions(mcpClusteringLogic)

    if (sortedClusters.length === 0 && !longTail) {
        return (
