...
async def run_pre_write_defensive_compact(
    ...
    A transient infra error (see `is_transient_maintenance_error`) is logged at warning instead
    of captured — the next sync's maintenance pass retries it from scratch, so it isn't a bug in
    this function, just a temporary blip talking to our own S3 bucket or app DB.
