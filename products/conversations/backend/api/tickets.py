...
class TicketViewSet(TaggedItemViewSetMixin, TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    ...
    def _get_view_filters(self, short_id: str) -> dict[str, Any]:
        ...
        filters_serializer = TicketViewFiltersSerializer(data=view.filters or {})
        if not filters_serializer.is_valid():
            raise ValidationError({"view": "This saved view's filters are invalid and cannot be applied."})
        return dict(filters_serializer.validated_data)
        ...
        parameters=[
            ...
            OpenApiParameter(
                ...
                description=(
                    ...
                    "Valid values: `persisted`, `escalated_with_best`, `escalated_no_reply`, "
                    "`skipped_unactionable`, `blocked_unsafe`, `blocked_unsafe_reply`, `in_progress`."
                ),
            ),
            OpenApiParameter(
                "snoozed",
