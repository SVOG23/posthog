...
import { commentToChatMessage, sourceColorForIndex } from './conversationMessages'
...
export interface supportTicketSceneLogicValues {
    ...
    mergedConversationsLoading: boolean
    mergedTickets: MergedTicketSummary[]
...
export interface supportTicketSceneLogicActions {
    ...
    loadMergedConversations: () => {
        value: true
    }
    loadMergedConversationsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
    ...
    loadMergedConversationsSuccess: (
        mergedConversations: {
            messages: CommentType[]
            ticket: Ticket
        }[],
    ...
    setVisibleMergedTicketIds: (ids: string[]) => {
        ids: string[]
    }
    showAllMergedTickets: () => {
        value: true
    }
...
export const supportTicketSceneLogic = kea<supportTicketSceneLogicType>([
    ...
    actions({
        ...
        // Merged-ticket message interleaving
        setMergedTicketVisibility: (ticketId: string, visible: boolean) => ({ ticketId, visible }),
        setVisibleMergedTicketIds: (ids: string[]) => ({ ids }),
    ...
    loaders(({ values, props }) => ({
        ...
        mergedConversations: [
            [] as { ticket: Ticket; messages: CommentType[] }[],
            {
                loadMergedConversations: async (): Promise<{ ticket: Ticket; messages: CommentType[] }[]> => {
                    const ids = values.visibleMergedTicketIds
                    if (ids.length === 0) {
                        return []
                    }
                    try {
                        return await Promise.all(
                            ids.map(async (id) => {
                                const [ticket, response] = await Promise.all([
                                    api.conversationsTickets.get(id),
                                    api.comments.list({ scope: 'conversations_ticket', item_id: id }),
                                ])
                                return { ticket, messages: (response.results || []).reverse() }
                            })
                        )
                    } catch {
                        lemonToast.error('Failed to load merged ticket messages')
                        return values.mergedConversations
                    }
                },
            },
        ],
    ...
    reducers({
        ...
        visibleMergedTicketIds: [
            [] as string[],
            {
                setMergedTicketVisibility: (state: string[], { ticketId, visible }) =>
                    visible ? Array.from(new Set([...state, ticketId])) : state.filter((id) => id !== ticketId),
    ...
    selectors({
        ...
        // Pills only make sense once more than one ticket's messages are on screen.
        showSourcePills: [
            (s) => [s.visibleMergedTicketIds],
    ...
    listeners(({ actions, values, props, cache }) => ({
        ...
        showAllMergedTickets: () => {
            actions.setVisibleMergedTicketIds(values.mergedTickets.map((t) => t.id))
        },
        hideAllMergedTickets: () => {
            actions.setVisibleMergedTicketIds([])
        },
        setMergedTicketVisibility: () => {
            actions.loadMergedConversations()
        },
        setVisibleMergedTicketIds: () => {
            actions.loadMergedConversations()
        },
